...
public class Files {
	...
	public static Boolean eliminaFichero(String f) {
		File file = new File(f);
		if (file.exists()) {
			file.delete();
			return true;
	...
	public static <T extends Serializable> void guardaString(String o, String f) {
		if (Files.existeFichero(f).booleanValue()) {
			Files.eliminaFichero(f);
		} else {
			String directories = f.substring(0,f.lastIndexOf("\\"));
